...
def get_target_date():
    ...
    if override:
        print(f"⚠️ TEST MODE: Overriding date to {override}")
        return override
...
def execute_strategy(analysis, client, dry_run=False):
    ...
    if zone == 1:
        ...
        total_buy = round(fresh_input + drain_amt, 2)
        
        print(f"🚀 BUY SIGNAL: ${total_buy}")
        if cash >= total_buy:
            if not dry_run:
        ...
        print(f"⚖️ NEUTRAL: Standard DCA ${DEFAULT_BASE_DCA}")
        if cash >= DEFAULT_BASE_DCA:
            if not dry_run:
                order = client.submit_order(MarketOrderRequest(symbol=SYMBOL, notional=DEFAULT_BASE_DCA, side=OrderSide.BUY, time_in_force=TimeInForce.GTC))
                result_log.update({'action': 'buy', 'amount': DEFAULT_BASE_DCA, 'id': str(order.id)})
            else:
                result_log.update({'action': 'buy_dry_run', 'amount': DEFAULT_BASE_DCA})

    elif zone == 3:
        if btc_qty > 0:
            sell_qty = round(btc_qty * (DEFAULT_SELL_FACTOR / 100.0), 8)
            if sell_qty > 0:
                print(f"🔻 SELL SIGNAL: {sell_qty} BTC")
                if not dry_run:
